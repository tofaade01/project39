  ...
  app.post("/user/create", async (req, res) => {
    // Async untuk concurrency, request dan responds
    try {
      const { title, caption, channel, date, time, totalBroadcast } = req.body; // Nerima dari frontend
      const payload = { title, caption, channel, date, time, totalBroadcast }; // Untuk menyimpan ketiga variabel menjadi satu paket
      const broadcast = await upBlast(payload);

      userQuery.createBroadcast(broadcast).then((broadcast) => {
        res.status(202).json({ message: "Success create new broadcast!", broadcast}); 
      });
  ...
  async function upBlast(payload){
    try{
      ...
      if (payload.date >= dateTimeISO) { // Membandingkan hanya tanggal, tanpa waktu
  ...
  async function register(payload) {
    try {
      ...
      const isValid = allowedDomains.some((domain) => payload.email.endsWith(domain));
  ...
  async function login(payload) {
    try {
      ...
      const isValidPassword = bcrypt.compareSync(payload.password, checkUser[0].password); // Check pass dengan db udah sama atau ga
      if (!isValidPassword) {
        throw new Error("Invalid email or password");
      }
